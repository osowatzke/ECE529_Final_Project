...
\end{abstract}
	...
	\subsection{Root-MUSIC}
		...
		After performing matrix multiplication, the resulting polynomial will include powers of $z$ from $-M+1$ to $M-1$.
		%
		\begin{equation}
		...
		\end{equation}
		%
		Equation (\ref{root_music_summation}) can be further simplified to the form given in \cite{doa_algorithms_raghu}
		%
		\begin{equation}
		...
		When the polynomial given by Equation (\ref{root_music_summation_simplified}) is zero, the spatial spectrum will be maximized. Therefore, we need to find the roots of the polynomial given by Equation (\ref{root_music_summation_simplified}). The roots inside the unit circle with magnitude closest to 1 are most of interest. 

...
\end{abstract}
	...
	\subsection{MUSIC}
		...
		The Multiple Signal Classification (MUSIC) algorithm takes an eigen-decomposition of the spatial auto-correlation matrix to produce orthogonal signal and noise subspaces. The eigenvectors of the resulting noise subspace are used to generate a spatial spectrum \cite{doa_algorithms_raghu}.
	...
	\end{equation}
	%
	In the above equation, $\Lambda = \diag\{\lambda_0,...,\lambda_{M-1}\}$ is the MxM diagonal eigenvalue matrix and $E=\{e_0,...,e_{M-1}\}$ is the MxM eigenvector matrix.
	
	% If the eigenvectors are sorted in ascending order (i.e. $\lambda_0 < ... < \lambda_{M-1}$, then the $N$ smallest eigenvalues $\{lambda_0,...,\lambda_{N-1}\}$ correspond to the noise subspace.
	
		The number of sources $D$ is must then be known to separate the a user specified parameter. can be estimated through
	
	
	The resulting noise subspace is used to generate 
	%\begin{equation}
	%	A = \begin{bmatrix}
	%		1 & 1 & \cdots & 1\\

...
\end{abstract}
	...
	\section{Simulation}
		...
		The performance of each of the above direction of arrival algorithms will be evaluated in simulation. A uniform linear array model will be used to generate data samples for each direction of arrival algorithm. This model will take the number of samples ($K$), the number of antenna elements ($M$), and the element spacing ($d$) as arguments. The received signal will be the sum of complex gaussian noise and Each of the received signals will be a complex exponential tuned at a user-specified frequency received at a user-specified angle.
